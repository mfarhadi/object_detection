...
def test_loss_package_exports_current_and_legacy_target_builders() -> None:
    assert AnchorFreeYoloLoss is not None
    assert build_anchor_free_targets is not None
...
def test_anchor_free_target_builder_uses_multi_positive_assignment() -> None:
    yolo_targets = torch.tensor([[0.0, 1.0, 0.5, 0.5, 0.6, 0.6]], dtype=torch.float32)

    targets = build_anchor_free_targets(
        yolo_targets=yolo_targets,
        batch_size=1,
        grid_h=4,
        grid_w=4,
